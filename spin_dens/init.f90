Module init
 ...
 !tot_interac storesthe number of possible interactions
 Integer(kind = int_kind) :: ios, num_spin_dens_set, poss_comb, main_cntr
 ...
 !jval stores the total (2S1S1 type )coefficient associated with each J
 !value
 ...
 !start and finish help determine the time taken during the program
 !execution
 Integer(kind = int_kind), Dimension(:,:), Allocatable :: singul
 !singul stores the location of singular dets of equations identified by
 !the
 !formal spin program. It essentially stores the data in the file singul
 Character (Len = 100) :: FileName1, FileName2, read_line, Outfile
 Logical :: EndOfFile, exis, stdv

Contains
 !Initialization Routines!
 Subroutine Init1()
   !!!!! Declaration and initailisation of local and global
   !variables !!!!!
        Character (Len = 1) :: buff
        Logical :: check, check1
        Integer(kind = int_kind) :: i1, j1, pos
   ...
   !!!!! Determining number of magnetic centres (i.e. metal
   !atoms) !!!!!
   ...
   !!!!! Determining how many sets of spin densities are there
   !for each metal centre !!!!!
   !!!!! This is achieved by counting the number of lines in the
   !spin density file   !!!!!
        i = 0
   ...
   !!!!! Determining the number of J values, reading them and
   !printing them!!!!!
        Read(11, '(a)', iostat = ios) read_line
        ...
        If (no_of_j_val >= num_spin_dens_set) Then
                ...
                Call Abort()
        End If
        !The addition of 1 has been made to make keep the last
        !value as the total
        !number of interaction that will help in comparisons
        !later
   ...
   !!!!! Reading in the Spin Hamiltonian !!!!!
        ...
        End Do
        jpos(pos) = i
        !The above line will store the total interaction in the
        !last position of
        !jpos
   !!!!! Calculating the weight of each interaction i.e., the
   !term -2S1S2
        Do i = 1, num_spin_dens_set
                Do j = 1, tot_interac
                        hamil(i, j) = 0
        ...
        Do i = 1, num_spin_dens_set
                Do j = 1, tot_interac
                        If(hamil1(j) > 0 .And. hamil2(j) > 0) Then
                                hamil(i,j) = -0.5 * spin(i, hamil1(j)) * spin(i, hamil2(j))
        !The above line is the same as 2S1S2 where Si =
        !0.5*(spin value obtained from the density file)
        !Si has not been divided by 2 each time so the overall
        !value has been
        !multiplied by 2 in the end to take that part into
        !account.
                        End If
                End Do
        End Do
    !!!!! Printing the Hamiltonian !!!!!
        ...
        !Write(12, *) 'Coefficient associated with each
        !interaction'
        !Write(12, '(a)', advance = 'no') ' '
        !Write(12, *) hamil1, hamil2
        !Do i = 1, num_spin_dens_set
        !        Do j = i+1, num_spin_dens_set
        !                If (hamil1(i) .ne. 0 .AND. hamil2(j)
        !                .ne. 0) then
        ...
        !Do i = 1, num_spin_dens_set
        !        Do j = 1, tot_interac
        !                If (hamil1(j) .ne. 0 .AND. hamil2(j)
        !                .ne. 0) then
        !                        Write(12, 249, advance='no')
        !                        hamil(i,j)
        !                End If
        !        End Do
        !        Write(12, *)
